locals {
  ...
  BROKER_SECURITY_GROUP_IDS = [local.SEC_GRP_PING_SSH_ID, local.SEC_GRP_AMQ_BROKER_ID]
  ROUTER_SECURITY_GROUP_IDS = [local.SEC_GRP_PING_SSH_ID, local.SEC_GRP_AMQ_BROKER_ID, local.SEC_GRP_AMQ_ROUTER_ID]
  ...
  # Using same zone/subnet as bak01 as the medium instance is not supported in zone3. Running with zone3
