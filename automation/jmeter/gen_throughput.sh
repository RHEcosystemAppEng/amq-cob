...
# ./gen_throughput.sh -i /Users/lrangine/Documents/Src/amq-cob/amq-cob/automation/jmeter/test_results.jtl
...
#      "$publisherThroughput" "$consumerThroughput" "$combinedThroughput"
    ...
    printf '{
       ...
       "throughputSec": {"publisher": "%s", "consumer": "%s", "combined": "%s"}
     }\n' \
      "$firstTimestamp" "$lastTimestamp" "$timeDiffMillis" "$timeDiffSeconds" \
      "$publisherCount" "$consumerCount" "$publisherSampleCount" "$consumerSampleCount" \
      "$publisherThroughput" "$consumerThroughput" "$combinedThroughput" > jtl_throughput_results.json
